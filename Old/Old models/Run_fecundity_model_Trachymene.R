...
SpDataFocal <- subset(SpData, Focal.sp.x == "T")
...
Intra <- as.integer(SpDataFocal$Trachymene.cyanopetala) #check that this is then excluded from competitor matrix
...
PrelimFit <- stan(file = "fecundity_model_env_cath.stan", data = c("N", "S", "Fecundity", "Intra", "SpMatrix", "Other", "shade", "phos"),
                  iter = 2000, chains = 3, thin = 3) #control = list(adapt_delta = 0.8, max_treedepth = 10))
# Things I'm trying 
# increasing iterations: still high Rhats at 5000 inter
# removed adapt delta and max treedepth at 5000 inter: less divergent interactions but Rhat still high 
## same at 6000 inter: even worse
## then thinning to two 
# increasing thinning: 
# adapt_delta:
