...
GermData <- subset(GermData, species.code =="WAAC" | species.code=="ARCA", 
                  select=c(species.code, present, Spent.total))
...
species <- rep(NA, N)
for(i in 1:N){
  species[i] <- which(SpeciesToUse == GermData$species.code[i])
}
