...
# define indexes for plots and blocks
  ...
  block_id <- as.numeric(as.factor(SpDataFocal$Block.x))
...
# compile data into list for model fitting
  data_prelim <- list(
    ...
    K_block = length(unique(block_id)),
    plot_id = plot_id,
    block_id = block_id,
...
# compile stan model
  bhfh_matform_repb <- stan_model(
    file = here("Sparse_model_fits/Stan/BH_FH_Preliminary2_re_plot_block.stan"),
    model_name = "bhfh_mf_rep"
  )
  
# fit the model
  mfit <- sampling(
    bhfh_matform_repb, data = data_prelim, 
...
# refit the model with the new constraint matrix Q and std_normal() priors on remaining alpha_hats
  bh_matform_repb <- stan_model(
    file = here("Sparse_model_fits/Stan/BH_Final2_re_plot_block.stan"),
