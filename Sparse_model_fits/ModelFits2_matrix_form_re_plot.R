...
# load helper functions
  ...
  mod <- "PSI-lambda_PSI-alpha_re_plot" # Phosphorous + shade + interaction for lambda, phosphorous + shade + interaction for alpha
...
# define indexes for plots and blocks
  plot_id <- as.numeric(as.factor(SpDataFocal$Plot.ID.x))
...
# compile stan model
  bhfh_matform_replot <- stan_model(
    file = here("Sparse_model_fits/Stan/BH_FH_Preliminary2_re_plot.stan"),
    model_name = "bhfh_mf_rep"
  )
  
# fit the model
  mfit <- sampling(
    bhfh_matform_replot, data = data_prelim, 
...
# refit the model with the new constraint matrix Q and std_normal() priors on remaining alpha_hats
  bh_matform_replot <- stan_model(
    file = here("Sparse_model_fits/Stan/BH_Final2_re_plot.stan"),
    model_name = "bh_mf_rep"
  )
...
# refit the model with new constraints
  mfit_final <- sampling(
    bh_matform_replot, data = data_final, 
    chains=3, iter = 3000, cores=3
    # control=list(adapt_delta = 0.9, max_treedepth=15)
  )
  
# perform some posterior predictive checks for the final model
  y_rep <- rstan::extract(mfit_final, pars = "y_rep")[[1]]
  ppc_list <- ppcs(y_rep, obs = Fecundity)
