...
# create matrix A of heterospecific abundances
  A <- as.matrix(AllSpAbunds[, which(names(AllSpAbunds) %in% SpToKeep)])
...
# create model matrices, X_alpha, X_lambda, X_eta (see "alt_model_form_ideas.pdf")
  env_covs <- c("Colwell.P", "Canopy")
  env_data <- SpDataFocal[, which(names(SpDataFocal) %in% env_covs)]
  env_data_std <- as.data.frame(apply(env_data, 2, scale))
...
# model matrix for LDGR can differ from X_alpha if it makes sense
   X_lambda <- model.matrix(
     ~ Colwell.P, 
     data = env_data_std
   )
# covariates for intraspecific competition should likely be the same as those
#   for the interspecific coefficients
   X_eta <- X_alpha
...
# save model fit
  saveRDS(mfit, file = "Sparse_model_fits/TRCY_Phos_peren_bhfh_matform.rds")
