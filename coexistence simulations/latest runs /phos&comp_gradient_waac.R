...
Nj[,c("Brachyscome.sp" , "Brachyscome.iberidifolia", "Crassula.colorata.or.exserta","Calotis.multicaulis", "Calotis.hispidula", 
      ...
      "Velleia.cycnopotamica", "Velleia.rosea", "Waitzia.acuminata")] <- Nj[,c("Brachyscome.sp" , "Brachyscome.iberidifolia", "Crassula.colorata.or.exserta","Calotis.multicaulis", "Calotis.hispidula", 
                                                                               ...
                                                                               "Lawrencella.rosea", "Nicotiana.rotundifolia", "Plantago.debilis", "Podolepis.canescens", "Podotheca.gnaphalioides", "Rhodanthe.laevis", "Rhodanthe.citrina",
                                                                               "Rhodanthe.manglesii", "Rhodanthe.polycephala", "Schoenia.cassiniana", "Schoenus.nanus", "Senecio.glossanthus", "Trachymene.cyanopetala",
                                                                               "Velleia.cycnopotamica", "Velleia.rosea", "Waitzia.acuminata")] # doubling native stem density
...
Nj8[,c("Schoenus.nanus")] <- Nj8[,c("Schoenus.nanus")]+160
...
# Nj7 ####
arca.into.observed7B <- array(data=NA, dim =c(plots, posteriors, ncol(env_gradient))) # make an empty matrix (array for env??)
...
for (r in 1:plots) {
  for (x in 1:posteriors) {
    for (e in 1:ncol(env_gradient)) {
      ...
      # invade WAAC
      arca_tp1 <- (1-germ[y])*surv[y]*1 + 
        1*germ[y]*lambdas/(1+sum(a_eij*Nj7B[r,]))
      #Nj needs to be array of plots by reserve by species and then subscripted to match a_eij [plot r and reserve 1]
      # calculate LDGR of WAAC
      arca.into.observed7B[r,x,e] <- log(arca_tp1/1)
...
# Nj8 ####
arca.into.observed8B <- array(data=NA, dim =c(plots, posteriors, ncol(env_gradient))) # make an empty matrix (array for env??)
...
for (r in 1:plots) {
  for (x in 1:posteriors) {
    for (e in 1:ncol(env_gradient)) {
      ...
      # invade WAAC
      arca_tp1 <- (1-germ[y])*surv[y]*1 + 
        1*germ[y]*lambdas/(1+sum(a_eij*Nj8B[r,]))
      #Nj needs to be array of plots by reserve by species and then subscripted to match a_eij [plot r and reserve 1]
      # calculate LDGR of WAAC
      arca.into.observed8B[r,x,e] <- log(arca_tp1/1)
...
data.melt6 <- melt(arca.into.observed7B, varnames = c("plot", "post", "e"), value.name = "ldgr")
data.melt6$e <- as.factor(data.melt6$e)
data.melt6$plot <- as.factor(data.melt6$plot)
data.melt6$comp <- "Nj7"

data.melt7 <- melt(arca.into.observed8B, varnames = c("plot", "post", "e"), value.name = "ldgr")
data.melt7$e <- as.factor(data.melt7$e)
data.melt7$plot <- as.factor(data.melt7$plot)
data.melt7$comp <- "Nj8"
...
dataB <- full_join(dataB, data.melt6)
dataB <- full_join(dataB, data.melt7)
...
Nj9[,c("Pentaschistis.airoides")] <- Nj9[,c("Pentaschistis.airoides")]+80
Nj10[,c("Pentaschistis.airoides")] <- Nj10[,c("Pentaschistis.airoides")]+160
...
# Nj9 ####
arca.into.observed9 <- array(data=NA, dim =c(plots, posteriors, ncol(env_gradient))) # make an empty matrix (array for env??)
...
for (r in 1:plots) {
  for (x in 1:posteriors) {
    for (e in 1:ncol(env_gradient)) {
      ...
      # invade ARCA
      arca_tp1 <- (1-germ[y])*surv[y]*1 + 
        1*germ[y]*lambdas/(1+sum(a_eij*Nj9P[r,]))
      
      # calculate LDGR of WAAC
      arca.into.observed9[r,x,e] <- log(arca_tp1/1)
...
# Nj10 ####
arca.into.observed10 <- array(data=NA, dim =c(plots, posteriors, ncol(env_gradient))) # make an empty matrix (array for env??)
...
for (r in 1:plots) {
  for (x in 1:posteriors) {
    for (e in 1:ncol(env_gradient)) {
      ...
      # invade ARCA
      arca_tp1 <- (1-germ[y])*surv[y]*1 + 
        1*germ[y]*lambdas/(1+sum(a_eij*Nj10P[r,]))
      
      # calculate LDGR of WAAC
      arca.into.observed10[r,x,e] <- log(arca_tp1/1)
...
data.melt9 <- melt(arca.into.observed9, varnames = c("plot", "post", "e"), value.name = "ldgr")
data.melt9$e <- as.factor(data.melt9$e)
data.melt9$plot <- as.factor(data.melt9$plot)
data.melt9$comp <- "Nj9"

data.melt10 <- melt(arca.into.observed10, varnames = c("plot", "post", "e"), value.name = "ldgr")
data.melt10$e <- as.factor(data.melt10$e)
data.melt10$plot <- as.factor(data.melt10$plot)
data.melt10$comp <- "Nj10"

data <- full_join(data.melt, data.melt1)
data <- full_join(data, data.melt2)
data <- full_join(data, data.melt3)
data <- full_join(data, data.melt9)
data <- full_join(data, data.melt10)
