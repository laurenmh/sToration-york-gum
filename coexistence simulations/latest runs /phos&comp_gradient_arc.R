...
germination <- data.frame(arca=germ$p.1, waac=germ$p.2) # order is arca, waac
...
ex.stems <- Nj3[,c("Arctotheca.calendula", "Calotis.hispidula", "Ehrharta.longiflora", "Hypochaeris.glabra", "Lysimachia.arvensis", "Monoculus.monstrosus",
                   "Oxalis.sp", "Parentucellia.latifolia", "Pentaschistis.airoides", "Petrorhagia.velutina", "Sonchus.oleraceus", "Urospermum.picroides",
                   "Ursinia.anthemoides", "Vulpia.bromoides")]
mean(colSums(ex.stems))

# seeding natives (add any number) (all natives or can pick specific ones - eg deviation species from stan model)
Nj2[,c("Angianthus.tomentosus", "Brachyscome.iberidifolia", "Crassula.colorata.or.exserta", "Daucus.glochidiatus", "Gilberta.tenuifolia",
      "Gonocarpus.nodulosus", "Goodenia.berardiana", "Haloragis.odontocarpa", "Hyalosperma.glutinosum.subsp.glutinosum", "Hydrocotyle.pilifera", 
      "Lawrencella.rosea", "Nicotiana.rotundifolia", "Plantago.debilis", "Podolepis.canescens", "Podotheca.gnaphalioides", "Rhodanthe.laevis",
      "Rhodanthe.manglesii", "Rhodanthe.polycephala", "Schoenia.cassiniana", "Schoenus.nanus", "Senecio.glossanthus", "Trachymene.cyanopetala",
      "Velleia.cycnopotamica", "Velleia.rosea", "Waitzia.acuminata")] <- Nj2[,c("Angianthus.tomentosus", "Brachyscome.iberidifolia", "Crassula.colorata.or.exserta", "Daucus.glochidiatus", "Gilberta.tenuifolia",
                                                                               "Gonocarpus.nodulosus", "Goodenia.berardiana", "Haloragis.odontocarpa", "Hyalosperma.glutinosum.subsp.glutinosum", "Hydrocotyle.pilifera", 
                                                                               "Lawrencella.rosea", "Nicotiana.rotundifolia", "Plantago.debilis", "Podolepis.canescens", "Podotheca.gnaphalioides", "Rhodanthe.laevis",
                                                                               "Rhodanthe.manglesii", "Rhodanthe.polycephala", "Schoenia.cassiniana", "Schoenus.nanus", "Senecio.glossanthus", "Trachymene.cyanopetala",
...
Nj3[,c("Arctotheca.calendula", "Calotis.hispidula", "Ehrharta.longiflora", "Hypochaeris.glabra", "Lysimachia.arvensis", "Monoculus.monstrosus",
       ...
       "Ursinia.anthemoides", "Vulpia.bromoides")] <- Nj3[,c("Arctotheca.calendula", "Calotis.hispidula", "Ehrharta.longiflora", "Hypochaeris.glabra", "Lysimachia.arvensis", "Monoculus.monstrosus",
                                                             "Oxalis.sp", "Parentucellia.latifolia", "Pentaschistis.airoides", "Petrorhagia.velutina", "Sonchus.oleraceus", "Urospermum.picroides",
                                                             "Ursinia.anthemoides", "Vulpia.bromoides")]*0
Nj4[,c("Arctotheca.calendula", "Calotis.hispidula", "Ehrharta.longiflora", "Hypochaeris.glabra", "Lysimachia.arvensis", "Monoculus.monstrosus",
       "Oxalis.sp", "Parentucellia.latifolia", "Pentaschistis.airoides", "Petrorhagia.velutina", "Sonchus.oleraceus", "Urospermum.picroides",
...
arca.into.observedB <- array(data=NA, dim =c(plots, posteriors, ncol(env_gradient), length(Neighhoods)))
# set reserve  
res=1
# Nj ####
for (r in 1:plots) {
  for (x in 1:posteriors) {
    for (e in 1:ncol(env_gradient)) {
      ...
      #log_a_eij <- (1-Intra)*arca.phos$alpha_generic[x,1]+Intra*arca.phos$alpha_intra[x,1] +Inclusion_ij*arca.phos$alpha_hat_ij[x,,]+ (arca.phos$alpha_intra[x,2]+Inclusion_eij*arca.phos$alpha_hat_eij[x,,])*env_gradient[r,e] 
      #Gives a 2x45 matrix of log_alpha_e,i,j for speciesxlocation
        # hard code in reserve 
...
dataB$comp <- revalue(dataB$comp, c("1"="observed", "2"="double.nat", "3"="remove.ex", "4"="double.ex"))

saveRDS(dataB, file = "Sim data/phos&comp_gradient_arca_bd.rds")
...
data$comp <- revalue(data$comp, c("1"="observed", "2"="triple.nat", "3"="remove.ex", "4"="double.ex"))

saveRDS(data, file = "Sim data/phos&comp_gradient_arca_pj.rds")
