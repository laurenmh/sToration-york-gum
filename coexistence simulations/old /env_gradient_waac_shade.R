...
surv=survival$waac
lambda = waac.shade$lambdas
alphas = waac.shade$alphas
...
waac.into.observed2 <- array(data=NA, dim =c(plots, posteriors, ncol(env_gradient))) # make an empty matrix (array for env??)
...
for (r in 1:plots) {
  for (x in 1:posteriors) {
    for (e in 1:ncol(env_gradient)) {
      ...
      #Inclusion_ij[,#]<-0 # based on the species we want to exclude?
      log_a_eij <- alphas[x,1]+Inclusion_ij*waac.shade$alpha_hat_ij[x,,]+ (alphas[x,2]+Inclusion_eij*waac.shade$alpha_hat_eij[x,,])*env_gradient[r,e] 
      ...
      waac.into.observed2[r,x,e] <- log(waac_tp1/1)
...
# with no comp
waac.into.observed3 <- array(data=NA, dim =c(plots, posteriors, ncol(env_gradient))) # make an empty matrix (array for env??)
  ...
  for (x in 1:posteriors) {
    for (e in 1:ncol(env_gradient)) {
      ...
      waac_tp1 <- (1-germ[y])*surv[y]*1 + 
        1*germ[y]*lambdas/(1+sum(a_eij*Nj))
      # calculate LDGR of WAAC
      waac.into.observed3[r,x,e] <- log(waac_tp1/1)
...
data.melt <- melt(waac.into.observed2, varnames = c("plot", "index", "e"), value.name = "ldgr")
data.melt$e <- as.factor(data.melt$e)
data.melt$plot <- as.factor(data.melt$plot)
data.melt1 <- melt(waac.into.observed3, varnames = c("plot", "index", "e"), value.name = "ldgr")
...
#ggsave(filename = file.path("Figures","shade_grad_waac_bend.pdf"))
ggsave(filename = file.path("Figures","shade_means_waac_bend.pdf"), width = 7, height = 7)
# perenjori ---------------------------------------------------------------------------
...
ggplot(NULL) +
  ...
  scale_y_discrete(labels =c("-2 sd", "-1 sd", "sd", "+1 sd", "+2 sd"))+
  theme_ridges(center = T)
#ggsave(filename = file.path("Figures","shade_grad_waac_pj.pdf"))
ggsave(filename = file.path("Figures","shade_means_waac_pj.pdf"), width = 7, height = 7)
