...
# Set the figure name
FigName <- here("Topher model fits/TRCY.pdf")
...
# Phosphorous
load(here("Topher model fits/TRCY_Phos_FinalFit.rdata"))
...
# Canopy cover
load(here("Topher model fits/TRCY_Shade_FinalFit.rdata"))
Post <- rstan::extract(FinalFit)
...
# Note: TRCY only has specifics in the intercept, not slope, so the code below (and directly above) takes advantage
# of that.
for(i in 1:EnvLength){
  # First phosphorous
  ...
  for(j in 1:nrow(PhosSpecifics)){
    CurRow <- PhosSpecifics[j,1]
    CurCol <- PhosSpecifics[j,2]
    SpPost <- exp(LogGenericPost + Phos$Post$alpha_hat_ij[,CurRow, CurCol])
    SpecificPlot[1,j,1,i] <- mean(SpPost)
    SpecificPlot[1,j,2:3,i] <- HDInterval::hdi(SpPost)
  }
  ...
  for(j in 1:nrow(ShadeSpecifics)){
    CurRow <- ShadeSpecifics[j,1]
    CurCol <- ShadeSpecifics[j,2]
    SpPost <- exp(LogGenericPost + Shade$Post$alpha_hat_ij[,CurRow, CurCol])
    SpecificPlot[2,j,1,i] <- mean(SpPost)
    SpecificPlot[2,j,2:3,i] <- HDInterval::hdi(SpPost)
  }
  GenericPost <- exp(LogGenericPost)
...
pdf(file = FigName, width = 8, height = 6, onefile = FALSE, paper = "special")
  ...
  axis(side = 2, at = seq(0, 120, by = 5), labels = FALSE, tcl = -0.25)
  for(i in 1:2){
  ...
  for(i in 1:8){
    lines(x = PlotPhos, y = SpecificPlot[1,i,1,], lwd = 1.5, col = AlphaCols)
  }
  ...
  # Now the deviating alphas
  for(i in 1:nrow(ShadeSpecifics)){
    lines(x = PlotShade, y = SpecificPlot[2,i,1,], lwd = 1.5, col = AlphaCols)
  }
